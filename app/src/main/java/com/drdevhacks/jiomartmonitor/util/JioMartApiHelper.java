    ...
    private static void parseResponse(String body, Product product, StockResult result) {
        try {
            ...
            if (root.has("store_meta")) {
                ...
                boolean ownStoreHit  = false;
                boolean otherExists  = meta.length() > 0;
                ...
                // store_meta existed but NONE of our stores are in it
                if (otherExists && !ownStoreHit) {
                    result.setNotAvailableAtLocation(true);
                    return;
                }
            }
            ...
            if (root.has("sizes")) {
                ...
                if (anyFound || ownStoreDataExists) {
                    for (Integer sid : storeIds)
                ...
                // Sizes had per-store data but none matched our stores
                if (otherStoreDataExists && !ownStoreDataExists) {
                    result.setNotAvailableAtLocation(true);
                    return;
                }

                // ── Strategy 3: total sellable_quantity (global fallback) ────
                int total     = 0;
                for (int i = 0; i < sizes.length(); i++) {
                    JSONObject sz = sizes.getJSONObject(i);
